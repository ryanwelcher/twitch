...
(function () {
  ...
  renderPollBars(voteBars); // check to see if the user has already voted

  if (!window.localStorage.getItem('voted')) {
    voteButtons.forEach(button => {
      button.addEventListener('click', () => {
        // 1. Register a vote
        registerVote(button); // 2. Disable the buttons

        disableButtons(voteButtons); // 3. Redraw the vote bars

        renderPollBars(voteBars); // 4. Display the message
